...
(function(global, factory) {
  ...
  Sprite3D.prototype = Object.assign(Object.create(Object3D.prototype), {
    ...
    updateM: function() {
      ...
      for (var i in this.__mat) {
        switch (i) {
          case "bothsides":
            this.el.style[prefix + "BackfaceVisibility"] = this.__mat[i]
              ? "visible"
              : "hidden";
            break;
          case "image":
            this.el.style["background" + firstUper(i)] =
              this.__mat[i] !== "" ? "url(" + this.__mat[i] + ")" : "";
            break;
          default:
    ...
    updateO: function() {
      ...
      this.el.style[prefix + "TransformOrigin"] =
    ...
    updateT: function() {
      ...
      this.el.style[prefix + "Transform"] =
        ...
        _S0 +
        "(" +
        (fixed2(this["rotation" + _S0]) % 360) +
        "deg) " +
        "rotate" +
        _S1 +
        "(" +
        (fixed2(this["rotation" + _S1]) % 360) +
        "deg) " +
        "rotate" +
        _S2 +
        "(" +
        (fixed2(this["rotation" + _S2]) % 360) +
        "deg) " +
        "scale3d(" +
        fixed2(this.scaleX) +
        ", " +
  ...
  Stage3D.prototype = Object.assign(Object.create(Sprite3D.prototype), {
    ...
    updateT: function() {
      this.fov = fixed0(
        (0.5 / Math.tan(((this.camera.fov * 0.5) / 180) * Math.PI)) *
          this.height
      );
      this.el.style[prefix + "Perspective"] = this.fov + "px";
      this.__rfix
        .position(fixed0(this.width / 2), fixed0(this.height / 2), this.fov)
        .rotation(
        ...
        .updateT();
      this.__pfix
        .position(-this.camera.x, -this.camera.y, -this.camera.z)
        .updateT();
      return this;
    },
  ...
  Plane3D.prototype = Object.assign(Object.create(Sprite3D.prototype), {
    ...
    updateF: function() {
      ...
      for (var i in this.__flt) {
        _flt +=
          this.__flt[i] !== "" ? i + "(" + this.__flt[i].join(",") + ")" : "";
      }
