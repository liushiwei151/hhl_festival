...
(function(global, factory) {
  ...
  function checkPropName(el, name, isDom) {
    ...
    if (isDom) {
      ...
      if (typeof el[name] === "string" || typeof el[name] === "number")
        return name;
    }
  ...
  function regValue(value) {
    ...
    if (_a)
      return { num: fixed(_a[2] + _a[3] + _a[4]), unit: _a[5], ext: _a[1] };
  ...
  function setProp(el, name, value, unit) {
    switch (name) {
      ...
      default:
        if (unit !== undefined)
          value = !cssNumber[hyphenize(name)]
            ? value + (unit || "px")
            : value + unit;
  ...
  function updateTransform(obj) {
    var _t = "";
    if (obj._jt_obj.x || obj._jt_obj.y || obj._jt_obj.z)
      _t +=
        "translate3d(" +
        obj._jt_obj.x +
        "," +
    ...
    if (
      obj._jt_obj.scaleX !== 1 ||
      obj._jt_obj.scaleY !== 1 ||
      obj._jt_obj.scaleZ !== 1
    )
      _t +=
        "scale3d(" +
        obj._jt_obj.scaleX +
        ...
        obj._jt_obj.scaleZ +
        ") ";
    if (obj._jt_obj.skewX || obj._jt_obj.skewY)
      _t += "skew(" + obj._jt_obj.skewX + "," + obj._jt_obj.skewY + ") ";
  ...
  function checkRem() {
    if (!tempDiv) {
      ...
      tempDiv.style.cssText =
        "border:0 solid; position:absolute; line-height:0px;";
    }
    if (!body) {
  ...
  Object.assign(tween.prototype, {
    initialize: function(el, time, fromVars, toVars, isDom) {
      ...
      this.endTime =
        this.startTime +
        this.repeatDelay * this.repeat +
        this.duration * (this.repeat + 1);
      this.curTime = null;
      this.lastTime = null;
    ...
    _update: function(time) {
      ...
      var _repeat = Math.min(
        this.repeat,
        Math.max(
          0,
          Math.floor(
            (this.curTime - this.startTime) / (this.duration + this.repeatDelay)
          )
        )
      );
      var _isRepeat = false;
      ...
      if (this.lastTime < this.curTime) {
        if (this.lastTime <= this.startTime && this.curTime > this.startTime) {
          if (!this.isSeek && this.onStart)
            this.onStart.apply(this.onStartScope, this.onStartParams);
        }

        if (_isRepeat && !this.isSeek && this.onRepeat)
          this.onRepeat.apply(this.onRepeatScope, this.onRepeatParams);

        if (this.lastTime < this.endTime && this.curTime >= this.endTime) {
          if (!this.isSeek && this.onEnd)
    ...
    _updateProp: function() {
        ...
        Math.max(
          ...
          (this.curTime === this.endTime
            ...
            : (this.curTime - this.startTime) %
              (this.duration + this.repeatDelay)) / this.duration
        )
      );
      ...
      if (!this.isSeek && this.onUpdate)
        this.onUpdate.apply(this.onUpdateScope, this.onUpdateParams);
    },
  ...
  function initData(obj) {
    ...
    for (var i in obj.fromVars) {
      ...
      obj.fromVars[i] =
        obj.fromVars[i] === null ? _o : checkValue(_o, obj.fromVars[i]);
      obj.toVars[i] =
        obj.toVars[i] === null
          ? _o
          : checkValue(obj.fromVars[i], obj.toVars[i]);
    }
  }
  ...
  Object.assign(JT, {
    ...
    fromTo: function(el, time, fromVars, toVars) {
      return time || toVars.delay
        ? createTween("fromTo", el, time, fromVars, toVars)
        : this.set(el, toVars);
    },

    from: function(el, time, fromVars) {
      return time || fromVars.delay
        ? createTween("from", el, time, fromVars, {})
        : this.set(el, fromVars);
    },

    to: function(el, time, toVars) {
      return time || toVars.delay
        ? createTween("to", el, time, {}, toVars)
        : this.set(el, toVars);
    },
